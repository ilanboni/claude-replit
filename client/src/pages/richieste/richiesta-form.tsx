...
import { ClienteForm } from "@/pages/clienti/cliente-form";
import {
  Form,
  FormControl,
  FormField,
...
export function RichiestaForm({ richiesta, clienteId, onSuccess, onCancel }: RichiestaFormProps) {
  ...
  const handleClientCreated = (cliente: Cliente) => {
  ...
  return (
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  Solo clienti registrati come compratori. Puoi anche creare il cliente alla fine.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
        ...
        {!clienteId && !form.watch("clienteId") && (
          <div className="flex items-center justify-center p-4 border border-dashed rounded-md">
            <div className="text-center">
              <p className="text-sm text-muted-foreground mb-2">Non hai ancora selezionato un cliente</p>
              <Button
                type="button"
                variant="outline"
                onClick={() => setShowNewClientDialog(true)}
                data-testid="button-new-client-bottom"
              >
                <UserPlus className="h-4 w-4 mr-2" />
                Crea Nuovo Cliente
              </Button>
